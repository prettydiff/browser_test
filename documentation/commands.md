...
1. `drial agent_data device`
   - Lists all device type agent data.
1. `drial agent_data user`
   - Lists all user type agent data.
1. `drial agent_data "desktop computer"`
   - Lists any agent whose names contain the search string
1. `drial agent_data "16f07e8ed7225f07912da48e0d51308e8fbf9dafc89d8accaa58abc1da8a2832a046082bfc2534eb4933a00bd673019cb90437c8a94cc0d0adaf9cff40c5083b"`
...
### Examples
1. `drial base64 encode string:"my string to encode"`
   - Converts the provided string into a base64 encoding.
1. `drial base64 encode path/to/file`
   - Converts the provided file into a base64 encoding.
1. `drial base64 encode http://file.from.internet.com`
   - Reads a file from a URI and outputs a base64 encoding.
1. `drial base64 decode string:"a big base64 string"`
...
1. `drial certificate /file/path/to/save`
...
1. `drial certificate remove /file/path/to/delete`
...
1. `drial certificate organization:"localhost"`
...
1. `drial certificate ca-name:"certificate"`
...
1. `drial certificate ca-domain:"localhost-ca"`
...
1. `drial test_browser mode:"self"`
...
1. `drial test_browser mode:"device"`
...
1. `drial test_browser mode:"user"`
...
1. `drial test_browser mode:"remote"`
...
1. `drial test_browser "C:\Program Files\Mozilla Firefox\firefox.exe" no_close`
...
### Examples
1. `drial test_service`
   - Runs tests service utility.
1. `drial test_service fs-copy`
...
1. `drial test_service "Copy from Remote Device to different Remote Device"`
...
### Examples
1. `drial test_simulation`
   - Runs tests against the commands offered by the services file.
1. `drial test_simulation help`
   - Filter the tests to run by supplying a text fragment to filter against test names.  For example if there are 6 service tests whose names contain that string then only those 6 tests will be evaluated.
1. `drial test_simulation "hash ~/share-file-systems list ignore ['node_modules'"`
   - Using quotes the filter argument may contain spaces and other non-alpha characters.
