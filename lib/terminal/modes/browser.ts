...
import filePathDecode from "../utilities/file_path_decode.js";
        ...
        methods: {
            ...
            respond: function terminal_test_application_browser_respond(item:testBrowserRoute): void {
                const errorCall = function terminal_test_application_browser_respond_errorCall(data:httpError):void {
                        log([`Error on ${data.callType} returning test index ${item.index} result`, data.error.toString()]);
                    },
                ...
                httpClient({
                    ...
                    requestError: function terminal_test_application_browser_respond_requestError(errorMessage:NodeJS.ErrnoException):void {
                        errorCall({
                            callType: "request",
                            error: errorMessage
                        });
                    },
                    responseError: function terminal_test_application_browser_respond_responseError(errorMessage:NodeJS.ErrnoException):void {
                        errorCall({
                            callType: "response",
                            error: errorMessage
                        });
