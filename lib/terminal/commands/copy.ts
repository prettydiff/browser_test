...
const copy = function terminal_commands_copy(params:copyParams):void {
                                ...
                                if (statError === null) {
                                    symlink(
                                        ...
                                        stat.isDirectory() === true
                                            ? "junction"
                                            : "file",
                                        function terminal_commands_copy_dirCallback_link_readLInk_stat_typeError(typeError:NodeJS.ErrnoException):void {
                                            types(typeError.toString());
                                        }
                                    );
                                    types(null);
                                } else {
    ...
    stat(params.destination, function terminal_commands_copy_stat(erStat:Error):void {
        ...
        if (erStat === null) {
            ...
            makeDir(params.destination, function terminal_commands_copy_stat_makeDir():void {
                directory(dirConfig);
            });
        }
    });
