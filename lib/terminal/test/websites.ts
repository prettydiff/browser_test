...
/* lib/terminal/test/websites - The code that allows testing of websites. */
...
const websites = function terminal_test_websites(options:websitesInput):void {
    ...
    vars.node.fs.stat(`${configuration.campaignLocation + vars.sep + options.campaignName}.js`, function terminal_test_websites_campaign(err:Error):void {
        if (err === null) {
            // @ts-ignore - this is working correct because es2020 is set in the tsconfig, but the ide doesn't see it
            import(`file:///${configuration.campaignLocation.replace(/\\/g, "/")}/${options.campaignName}.js`).then(function terminal_test_websites_campaign_promise(campaignData:campaignModule) {
